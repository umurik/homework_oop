...
def test_setter_decreasing_product(capsys):
    ...
    with patch('builtins.input', return_value='Y') as mock_input, patch('builtins.print') as mock_print:
        product.price = 11
    ...
    with patch('builtins.input', return_value='n') as mock_input, patch('builtins.print') as mock_print:
        product.price = 11
        mock_print.assert_called_once_with('Вы устанавливаете цену ниже, чем была')
        mock_input.assert_called_once_with('Продолжить?(Y/n)')
